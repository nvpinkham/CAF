...
#' @param groups categories vector
#' @return nmds object from vegan
#' @export
group.nmds <- function (otu, predictor, 
          groups, size = 1, color, 
          return.table = F, spider = T, legend.location = "bottomleft"){
  ...
  if (missing(color)) {
    color <- CAF::color.groups(predictor)
  }
  ...
  try({
    col.key <- aggregate(col.pick, list(predictor.pick), 
                         unique)
    ns <- table(predictor.pick)
    m <- match(col.key$Group.1, names(ns))
    col.key$ns <- ns[m]
    legend(legend.location, fill = col.key$x, legend = paste0(col.key$Group.1, 
                                                              " (n=", col.key$ns, ")"), bty = "n")
    if (spider) {
      ordispider(nmds.pick, predictor.pick, label = F, 
                 lwd = 1, col = "darkgrey")
    }
    else {
      ordiellipse(nmds.pick, predictor.pick, label = F, 
                  lwd = 1, col = col.key$x)
    }
  })
  ...
  set.seed(42)
  res <- vegan::adonis2(vegdist(otu.pick[p, ]) ~ predictor.pick[p], 
                        permutations = 9999)
  bres <- betadisper(vegdist(otu.pick[p, ]), predictor.pick[p])
  bres$group.distances
  predictor.unique <- unique(predictor.pick)
  otu.agg <- aggregate(otu.pick, by = list(predictor.pick), 
                       FUN = mean)
  row.names(otu.agg) <- otu.agg$Group.1
  otu.agg$Group.1 <- NULL
  centroid.dist <- vegdist(otu.agg)
  print(centroid.dist)
  res1 <- c(round(res$`Pr(>F)`[1], 4), round(res$R2[1], 3), 
            round(res$F[1], 3), round(max(centroid.dist), 3), length(predictor.pick))
  names(res1) <- c("p.val", "R2", "f.stat", "distance.between.centroids", 
                   "n")
  title(paste0("PERMANOVA\np val =", res1[1], ", R2 =", res1[2], 
               ", f stat =", res1[3], ", n=", res1[5], "\n max BC between centroids =", 
               res1[4]))
  if (return.table) {
    return(res1)
  }
